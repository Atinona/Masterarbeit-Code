...
def berechneALundUV(k, n, T, dt, r, S, alpha, m, sigma_r, B, N0, EK, i_mon, gl, gf, xli, gamma, sigma_BS,
    ...
    US = 0  # Überschuss
    UV = np.zeros(T - 1)  # Umschichtungsvolumen
    ...
    for t in range(1, T):
        ...
        if gf * V < BG:
            ...
            V_GF = BG / gf
        V_FF = V - V_KS - V_GF
        UV[t - 1] = V_KS - V_KS_alt  # Umschichtungsvolumen bestimmen
        ...
        if (V_KS_tat < 0):
            ...
            return 0, UV         # ggf. Abbruch bei Insolvenz
        V_A_alt = V_A
        V_A = 0.1 * V_KS_tat
...
for gl in np.arange(0.7, 1.01, 0.1):
    ...
    berechnung = Parallel(n_jobs=-1)(
        delayed(berechneALundUV)(
            k, n, T, dt, r1, S1, alpha, m, sigma_r, B, N0, EK, i_mon, gl, gf, xli, gamma,
            sigma_BS, delta, psi, N_storno, N_diffStorno
        ) for k in range(K)
    )
    V_array, UV_array = zip(*berechnung)
    V_array = np.array(V_array)
    np.savetxt("V_" + str(gl) + r".txt", V_array)   # enthält die Ablaufleistungen aller Simuletionen
    np.savetxt("UV_" + str(gl) + r".txt", UV_array) # enthält den Verlauf des Umschichtungsvolumens für alle
                                                    # Simulationen
    print(f"Erwartungswert Ablaufleistung: {np.mean(V_array)}")
    print(f"Standardabweichung Ablaufleistung: {np.std(V_array)}")
